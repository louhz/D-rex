...
class HandEnv:
    ...
    def compute_object_contacts(self):
        ...
        for i in range(self.mj_data.ncon):
            ...
            n1 = mj.mj_id2name(self.mj_model, mj.mjtObj.mjOBJ_GEOM, g1)
            n2 = mj.mj_id2name(self.mj_model, mj.mjtObj.mjOBJ_GEOM, g2)
            if ("cookie" in n1) or ("cookie" in n2):
                contact_count += 1
...
class GraspMLP(nn.Module):
    """
    GraspMLP updated with ONNX-based CNN backbone.
    Outputs action, reward, and force predictions.
    """
    def __init__(self, input_dim=24, hidden_dim=256, action_dim=16, reward_dim=2,force_dim=1):
        super().__init__()

        cnn_output_dim = 16 * 7 * 7  # Adjust if necessary
    ...
    def forward(self, x):
        ...
        returns:
          ...
          pred_reward:  (batch_size, reward_dim)
          pred_force:   (batch_size, force_dim)
        """
        features = x
        shared_feat = self.shared(features)

        pred_actions = self.action_head(shared_feat).mean(dim=1)
        # pred_reward = self.reward_head(shared_feat)
        # pred_force = self.force_head(shared_feat)
...
def main():
    ...
    xml_file = "_data/leap_hand/scene_cookie.xml"
    gt_mesh_path='_data/leap_hand/assets/cookie.ply'
    gt_control_path='cookie/leap_batched_output.npy'
    ...
    train_phase_1(model, phase1_loader, optimizer, epochs=1000, device=device)
    ...
    # Save the model
    model_save_path = "cookie/grasp_model_with_validation_withmass_200g.pth"
